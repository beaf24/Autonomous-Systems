...
max_x_size = int(size_info[0]) + np.abs(int(size_info[1])) + init_x + int(init_x/2)
max_y_size = int(size_info[2]) + np.abs(int(size_info[3])) + init_y + int(init_y/2)
...
for cell in data:
    ...
    if x > max_x_size + init_x or x < 0:
        print("x" + str(x))
    if y > max_y_size + init_y or y < 0:
        print("y" + str(y))
