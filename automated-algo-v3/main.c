...
int main(int argc, char **argv) {
    ...
    // sscanf(argv[1], "%lf", &meter_permap_algo.png_pixel);
    

    double p_occ_start=0.65;
    double p_occ_end=0.80;
    double p_occ_increment=0.01;
    double p_free;
    double l_occ;
    double l_free;
    ...
    // Iterate over the range of log-odds values
    for (double p_occ = p_occ_start; p_occ <= p_occ_end; p_occ += p_occ_increment) {

        // Reset the linked list and other variables for each iteration
        current = head;
        ...
        p_free = 1 - p_occ;
        l_occ = log(p_occ / p_free);    
        l_free = log(p_free / p_occ);
        printf("l_occ in v3 is %f for p_occ %f .\n", l_occ, p_occ);
        printf("l_free in v3 is %f for p_occ %f .\n", l_free, p_occ);
        ...
        // Rewind the file to the beginning for the next iteration
        rewind(file);
        ...
        char file_name[30];
        snprintf(file_name, sizeof(file_name), "export_data/export_%.2f.txt", p_occ);

        // fprintf(size ,"%d %d %d %d", x_max, x_min, y_max, y_min);
        // fclose(size);

        // Open the file for writing
        FILE *export = fopen(file_name, "a");
        ...
        for(int i = 0; i < size_x; i++){
            for (int j = 0; j < size_y; j++)
            {
                ...
                fprintf(export, "%f\n", mat[i][j]);
                // if(p_occ != 0.70){           
                //     printf("size_y is %d", size_y);}

                // printf("%f in %s\n", mat[i][j], file_name);  
                // printf("size y is %d and x is %d\n", size_y, size_x);                           
                continue; 
                }
